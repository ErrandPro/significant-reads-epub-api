...
def convert_doc_to_docx(doc_path: str) -> str:
    ...
    Prerequisites (install once on the server):
        ...
        RHEL/CentOS   : sudo yum install -y libreoffice
        macOS (brew)  : brew install libreoffice
    ...
    The caller is responsible for cleaning up the directory when done:
        import shutil; shutil.rmtree(os.path.dirname(returned_path), ignore_errors=True)
...
def extract_rich_chapters_from_docx(
    ...
    epub_builder.py needs no changes:
        ...
        ───────────────────────────────────
        {
            ...
            "is_section": bool,           # True → rendered as <h2>
            "dropcap_char": str | None,   # optional
        }

    Heading mapping
    ───────────────
        Heading 1 / Title style  → new chapter boundary
        Heading 2 / Heading 3    → section heading (is_section = True)
        Paragraph with very large font (>1.5× body) → chapter boundary
        Paragraph with large font (>1.15× body)     → section heading

    Floating text boxes in the Word document are emitted as "sidebar" blocks.
    """
    try:
        from docx import Document
        ...
        def _para_heading_level(para) -> int:
            """
            Return 0 (not a heading), 1 (chapter), or 2 (section).
            Checks Word style name first, then font-size heuristic.
            """
            ...
            # Pattern-based (Introduction, Chapter N, …)
        ...
        # ── 3. Collect floating text-boxes (sidebars) first ───────────────
        #
        # Floating (anchored) drawings that contain a text-box are emitted as
        # sidebar blocks.  We collect their XML element identity so we can
        # skip them when iterating body paragraphs.
        #
        sidebar_blocks: list[dict] = _extract_textboxes(doc, body_size)

        # ── 4. Iterate top-level body children ────────────────────────────
        #
        # We walk doc.element.body directly so tables are interleaved with
        ...
        for elem in doc.element.body:
            ...
            # ── Sectional properties / other elements — skip ───────────────
            else:
                continue
        ...
        # ── 5. Inject sidebar blocks after the chapter they belong to ──────
        #
        # Floating text boxes have no explicit position in the body iteration
        # so we append them to the first chapter as a trailing block cluster.
        # (A more precise approach would require matching page coordinates via
        # LibreOffice render — outside scope for now.)
        if sidebar_blocks and chapters:
            chapters[0][1].extend(sidebar_blocks)   # type: ignore[index]
...
def _docx_run_size(run) -> float | None:
    ...
    try:
        ...
        # Fall back to rPr/sz in raw XML
        ...
        if rpr is not None:
            ...
            if sz_node is not None:
                ...
                if val:
                    return int(val) / 2.0   # half-points → points
    except Exception:
        pass
    return None
...
def _extract_inline_images(para) -> list[dict]:
    ...
    for drawing in para._element.findall(
        f".//{{{_NS_W}}}drawing"
    ):
        # Only inline drawings (not anchored floating ones handled elsewhere)
        ...
        for blip in drawing.findall(f".//{{{_NS_A}}}blip"):
            ...
            try:
                ...
                if ext == "jpeg":
                    ext = "jpg"
                if len(blob) < 1024:                      # skip tiny icons
                    continue
...
def _extract_textboxes(doc, body_size: float) -> list[dict]:
    ...
    Anchored text boxes live in:
        w:drawing / wp:anchor / … / wps:txbx / w:txbxContent / w:p …
    """
    sidebars: list[dict] = []
    ...
    for anchor in doc.element.body.findall(
        f".//{{{_NS_WP}}}anchor"
    ):
